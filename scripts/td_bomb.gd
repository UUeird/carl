...
## Uses a static pool — call TDBomb.acquire() instead of instantiating directly.
...
static func prewarm(scene_root: Node, bomb_scene: PackedScene) -> void:
	_ensure_statics()
	for i in POOL_SIZE:
		var b: TDBomb = bomb_scene.instantiate()
		b._exploded = false
...
static func acquire(scene_root: Node, bomb_scene: PackedScene) -> TDBomb:
	if _pool.size() > 0:
...
func launch_bomb(from: Vector3, to: Vector3, speed: float, damage: float, aoe: float, damage_type: int = TDTower.DamageType.PHYSICAL) -> void:
	_from = from
	_to = to
	_damage = damage
	_aoe = aoe
...
func _spawn_blast() -> void:
	...
	scene.add_child(_BlastFade.new(ring, mat, _aoe))
